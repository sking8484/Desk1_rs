...
impl NDArrayMath {
    ...
    pub fn filter_svd_matrices(
        &self,
        svd_response: SvdResponse,
        min_significant_val: f64,
    ) -> SvdResponse {
        ...
        return SvdResponse {
            u: svd_response.u.clone(),
            sigma: new_sigmas,
            vt: svd_response.vt.clone(),
        };
    }
...
impl NDArrayHelper {
    pub fn build_svd_return_object(
        &self,
        svd_response: nalgebra::SVD<f64, nalgebra::Dyn, nalgebra::Dyn>,
    ) -> Result<SvdResponse, ShapeError> {
        let u: Array2<f64> = self
            .convert_nalgebra_to_ndarray(svd_response.u.expect("U Matrix should not be empty"))?
            .t()
            .to_owned();
        let sigma: Array2<f64> =
            Array2::from_diag(&self.convert_nalgebra_vec_to_ndarray(svd_response.singular_values)?);
        let vt: Array2<f64> = self
            .convert_nalgebra_to_ndarray(svd_response.v_t.expect("V_T matrix should not be empty"))?
            .t()
            .to_owned();

        return Ok(SvdResponse { u, sigma, vt });
    }
    ...
    pub fn convert_ndarray_to_polars(
        &self,
        array: Array2<f64>,
        ...
        let df: DataFrame = DataFrame::new(
            array
                ...
                .into_iter()
                .enumerate()
                .map(|(i, col)| Series::new(&column_names[i], col.to_vec()))
                .collect::<Vec<Series>>(),
        )
        .unwrap();
        return Ok(df);
    }
    pub fn convert_nalgebra_to_ndarray(
        &self,
    ...
    pub fn convert_nalgebra_vec_to_ndarray(
        &self,
        vector: nalgebra::DVector<f64>,
    ) -> Result<Array1<f64>, ShapeError> {
        let ndarray_array =
            Array1::from(vector.as_slice().iter().map(|&x| x).collect::<Vec<f64>>());
...
mod test_ndarray_math {
    ...
    fn test_individual_svd_elements() {
        ...
        let sigma = array![[40.0_f64.powf(0.5), 0.0], [0.0, 10.0_f64.powf(0.5)]];
        ...
        if let Ok(val) = svd_output {
            assert_eq!(
                val.u.map(|&x| ((x * 1000.0).round() / 1000.0).abs()),
                U.map(|&x| ((x * 1000.0).round() / 1000.0).abs())
            );
            assert_eq!(
                val.sigma.map(|&x| (x * 1000.0).round() / 1000.0),
                sigma.map(|&x| (x * 1000.0).round() / 1000.0)
            );
            assert_eq!(
                val.vt.map(|&x| ((x * 1000.0).round() / 1000.0).abs()),
                vt.map(|&x| ((x * 1000.0).round() / 1000.0).abs())
            );
        }
    }
    ...
    fn test_svd_combines_to_original() {
        ...
        if let Ok(val) = svd_output {
            ...
            assert_eq!(
                recombined_matrix.map(|&x| { (x * 1000.0).round() / 1000.0 }),
                array
            );
        }
    }
    ...
    fn test_filter_all_svd() {
        let array = array![[4.0, 0.0], [3.0, -5.0]];
        let svd_output = NDArrayMath {}.svd(array.clone());
        let expected_sigma = array![[0.0, 0.0], [0.0, 0.0]];
        let filtered_output =
            NDArrayMath {}.filter_svd_matrices(svd_output.expect("SVD Output should be ok"), 1.0);
    ...
    fn test_filter_none_svd() {
        ...
        let filtered_output =
            NDArrayMath {}.filter_svd_matrices(svd_output.expect("SVD Output should be ok"), 0.0);
        assert_eq!(
            filtered_output
                .sigma
                .map(|&a| (a * 1000.0).round() / 1000.0),
            sigma.map(|&a| (a * 1000.0).round() / 1000.0)
        );
    }
    ...
    fn test_filter_some_svd() {
        ...
        let sigma = array![[40.0_f64.powf(0.5), 0.0], [0.0, 0.0]];

        let filtered_output =
            NDArrayMath {}.filter_svd_matrices(svd_output.expect("SVD Output should be ok"), 0.5);
        assert_eq!(
            filtered_output
                .sigma
                .map(|&a| (a * 1000.0).round() / 1000.0),
    ...
    fn test_recombinded_svd() {
        ...
        assert_eq!(
            recombined_maxtrix.map(|&a| (a * 1000.0).round() / 1000.0),
            expected_matrix.map(|&a| (a * 1000.0).round() / 1000.0)
        );
    }

    #[test]
    fn test_build_linear_model() {
        let input_a = array![[1.0, 2.0], [1.0, 5.0], [1.0, 7.0], [1.0, 8.0]];
